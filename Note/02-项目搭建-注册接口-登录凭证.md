...
一般使用 md5 对密码进行加密。导入 Node 中，内置的库 `crypto`：
...
web 开发中，使用最多的协议，是 http；但 http 是一个无状态的协议。

事实上，每个 http 请求，对于服务器来说，都是单独的请求，和之前请求过什么，没有关系。

所以，即使在一个站点登录了。之后的 http 请求，默认也没法携带用户信息，给到服务器。
...
- 有设置过期时间，并且过期时间不为 0 或者负数的cookie，是硬盘 cookie，需要手动或者到期时，才会删除；
