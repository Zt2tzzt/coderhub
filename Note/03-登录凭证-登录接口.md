...
在 Node 项目中，使用 _koa_ 框架；

在中间件中，为客户端（浏览器）设置 cookie。并设置过期时间。
...
- 设置一个 `secretKey`（默认使用“对称加密”），将前两部分（header、payload）的结果合并后，进行 *HMAC SHA256（HS256）*的算法；
...
这个时候，推荐使用非对称加密（_RS256_算法），来提高安全性，它需要用到：
