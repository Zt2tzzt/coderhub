...
服务器为浏览器设置 cookie 后，在浏览器发送请求时，会自动携带 cookie，前端不需要做任何操作。
...
明文的 cookie，容易被伪造。在开发中，一般使用 session
...
对于浏览器以外的其他客户端（比如 iOS、Android），必须手动的设置 cookie 和 session，是很麻烦的。
...
token 可以翻译为“令牌”；也就是在验证了用户账号和密码正确的情况，给用户颁发一个令牌；
...
- `alg`：采用的加密算法，默认是 _HMAC SHA256（HS256）_，采用**同一个密钥**进行加密和解密；
- `typ`：JWT，固定值，通常都写成 JWT 即可；
- 会通过 _base64Url_ 算法进行编码；

payload

- 携带的数据，比如：可将用户的 `id` 和 `name` 放到 payload 中；
...
> 在客户端，一般通过 _Bearer token_ 的形式，在请求的 `header` 中携带 token。
...
userRouter.get('/list', (ctx, next) => {
  ...
  try {
    const result = jwt.verify(token, publicKey, {
      algorithms: ['RS256'] // 在数组中，可以放入多个算法，jwt 会一个个尝试解密。
    })
    console.log('result:', result)
...
>
> - `fs.readFileSync(path)`中，`path` 是相对于项目启动目录，即项目根目录的相对路径。
