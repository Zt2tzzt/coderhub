...
- 比如 `moment` 表 `LEFT JOIN` `comment` 表后，又 `LEFT JOIN` `moment_label` 表；
- 那么 `moment` 如果又 `1` 条记录，该记录有 `3` 泰评论，又 `4` 个标签，那么查询出来有 `3 * 4 = 12` 个标签。
