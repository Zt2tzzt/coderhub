...
- 用于限制一个 origin 的文档，或者它加载的脚本，与另一个 origin 源的资源进行交互。
- 从而阻隔恶意文档，减少可能被攻击的媒介。
...
然而，随着前后端的分离开发模式的普及，服务器请求静态资源（html 文件、js 脚本文件），和服务器开发的 API 接口，往往是分离的；

或者是部署在不同的服务器上的；

这时，我们发现，浏览器访问静态资源服务器，和 API 接口服务器，往往不是同一个源（不同的主机或端口）。

浏览器发现，请求静态资源，和向 API 接口请求资源（XHR、Fetch），不是来自同一个源时（同源策略），就产生了跨域；

相反的，在请求静态资源，和通过 API 请求资源是同一个源时，是没有跨域问题的。
...
- 事实上，配置前端项目的 webpack 打包工具，去解决跨域问题，
- 本质上，也是在 webpack-server 的服务器中，配置了代理。
...
html 文件中，JS 脚本请求的 API 资源，也是 `localhost:8000`；
...
CORS 意为“**跨源资源共享（CORS，Cross-Origin Resource Sharing）**”：
- 它是一种基于 http header 的机制；
- 该机制，允许服务器标示，除自己以外的其它源（域、协议和端口），来告知览器，允许这些 origin 访问加载自己的资源。
...
所以，一般在开发阶段，采用下方的方案三，部署后采用下方的方案四。
